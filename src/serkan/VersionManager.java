...
public class VersionManager {
	...
	public synchronized Document getLineObject(String line) {
		...
		if (split != null) {
			...
			document.setRemoteversion(Float.valueOf(split[2]));
		}
		return document;
	}
	...
	public synchronized Float setVersionline(String filePath, String fileToVersion, String newFilePath)
			throws IOException {
		Float newFileVersion = Float.valueOf(Calendar.getInstance().getTimeInMillis());
		File file = new File(filePath + JADE_VERSION);
		if (!file.exists()) {
			file.createNewFile();
		}
